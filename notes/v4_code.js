...
Return: {"task_type": "create_employee|create_customer|create_product|create_invoice|register_payment|create_travel_expense|update_employee|update_customer|delete_employee|delete_customer|credit_note|create_department|create_project|create_voucher|unknown", "confidence": 0.0, "extracted_params": {}, "reasoning": ""}`;
  ...
  switch (plan.task_type) {
    ...
    case 'create_customer': {
      ...
      if (p.postalCode || p.city || p.address) {
        ...
        b.postalAddress.country = { id: 162 };
      }
    ...
    case 'create_voucher': {
      ...
      if (postings.length === 0) {
        const vPlan = await callGemini('Create Tripletex voucher postings for this task. Use Norwegian standard chart of accounts.\nCommon: 1500=Kundefordringer, 1920=Bank, 2400=Leverandorgjeld, 2700=Utg.mva, 3000=Salgsinntekt, 4000=Innkjop, 6300=Leie, 6800=Kontorkostnader, 7100=Bilkostnader\nTask: ' + task + '\nReturn: {"postings": [{"accountNumber": 1920, "amount": 1000, "isDebit": true, "description": "..."}]}', []);
        if (vPlan.postings) {
      ...
      if (postings.length > 0) {
        const r = await tx('POST', '/ledger/voucher', { date: vDate, description: p.description || task.substring(0, 100), postings });
        results.push(r); success = r.ok;
    ...
    case 'update_employee': {
      ...
      if (!emp) {
        ...
        if (s.ok && s.data && s.data.values && s.data.values.length > 0) {
          ...
          if (!emp && (searchFirst || searchLast)) emp = s.data.values[0]; // Only fallback if we had search terms
        }
      }
      if (emp) {
