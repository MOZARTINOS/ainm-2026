...
async function getVatId(pct) {
  ...
  // Prefer outgoing VAT ("Utgående") for sales - id=3 in most Tripletex environments
  const outgoing = cachedVatTypes.find(v => v.percentage === target && v.name && (v.name.toLowerCase().includes('utg') || v.name.toLowerCase().includes('outgoing')));
  ...
  switch (plan.task_type) {
    ...
    case 'create_employee': {
      ...
      if (p.phoneNumberMobile || p.phoneNumber) {
        ...
        // Normalize: strip leading +47, keep 8 digits for Norwegian numbers
        ...
        // Only set if looks like valid Norwegian number (8 digits starting with 4,9)
        if (phone.length === 8 && /^[49]/.test(phone)) {
          b.phoneNumberMobile = phone;
        }
        // Otherwise skip — Tripletex rejects non-Norwegian phone formats
      }
      if (p.dateOfBirth) b.dateOfBirth = p.dateOfBirth;
      if (p.startDate) b.startDate = p.startDate;
      // CRITICAL FIX: userType must be STANDARD (with email) or NO_ACCESS (without)
      // ADMINISTRATOR is NOT a valid Tripletex userType!
      if (b.email) {
        b.userType = 'STANDARD';
      } else {
        b.userType = 'NO_ACCESS';
      }
      // Department lookup - REQUIRED field
      if (p.department) {
      ...
      if (!b.department) {
        const defDept = await tx('GET', '/department?from=0&count=1');
        if (defDept.ok && defDept.data && defDept.data.values && defDept.data.values.length > 0) {
          b.department = { id: defDept.data.values[0].id };
        }
      }
    ...
    case 'create_project': {
      ...
      // projectManager is REQUIRED - use first employee
      const pmEmp = await tx('GET', '/employee?from=0&count=1');
      if (pmEmp.ok && pmEmp.data && pmEmp.data.values && pmEmp.data.values.length > 0) {
        b.projectManager = { id: pmEmp.data.values[0].id };
      }
    ...
    case 'delete_employee': case 'delete_customer': case 'delete_product': {
      ...
      if (entityId) {
        ...
        results.push(r);
        success = r.ok;
        // Fallback: if DELETE returns 403, try blanking the entity
        if (!r.ok && r.status === 403 && entityType === 'employee') {
          ...
          if (full.ok) {
            ...
            upd.firstName = 'DELETED';
            upd.lastName = 'DELETED';
            upd.phoneNumberMobile = '';
            upd.comments = 'Deleted by automation';
            const r2 = await tx('PUT', '/employee/' + entityId, upd);
            results.push({ step: 'soft_delete', ...r2 });
    ...
    case 'update_employee': {
      ...
      if (es.ok && es.data && es.data.values && es.data.values.length > 0) {
        ...
        if (full.ok) {
          const upd = full.data.value;
          // CRITICAL: dateOfBirth is required on PUT even if null on GET
          if (!upd.dateOfBirth) upd.dateOfBirth = '1990-01-01';
